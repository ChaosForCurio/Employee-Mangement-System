...
import {
  ...
  CheckSquare, FileText, LogOut, Award, Briefcase, Bell, ClipboardList, X
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-slate-900 text-slate-300 h-screen flex flex-col fixed left-0 top-0 border-r border-slate-800 z-40 transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1 py-4 custom-scrollbar">
          {menuItems.map((item) => (
            <NavLink
              ...
              onClick={() => setIsOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-lg transition-colors duration-200 group ${isActive ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 hover:text-white'}`
              }
            >
